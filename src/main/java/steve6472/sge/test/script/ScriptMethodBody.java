...
class ScriptMethodBody
{
	...
	void parse()
	{
		main : for (String s : body)
		{
			...
			{
				for (ScriptMethod method : ScriptTest.methods)
				{
	...
	{
		if (ScriptTest.checkBasicMethod(s, "print") || ScriptTest.checkBasicMethod(s, "printErr"))
		{
			return true;
		}
	...
	{
		// If is a method
		if (ScriptTest.checkBasicMethod(s, sm.name))
		{
			String[] parameters = ScriptTest.getParameters(s);
			...
			{
				scriptParameters[i] = ScriptTest.getParameterType(parameters[i]);
