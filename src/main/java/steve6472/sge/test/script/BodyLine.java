...
class BodyLine
{
	...
	BodyLine method(ScriptMethod sm)
	{
		...
		mc.parameters = ScriptTest.getParameters(text);
//
...
//		}
	...
	BodyLine builtinMethod()
	{
		...
		if (ScriptTest.checkBasicMethod(text, "print")) bmc.bmt = BMT.PRINT;
		if (ScriptTest.checkBasicMethod(text, "printErr")) bmc.bmt = BMT.PRINT_ERR;

		bmc.parameters = ScriptTest.getParameters(text);
	...
	BodyLine varDec()
	{
		...
		vd.type = ScriptTest.parseDataType(t);
...
//	}
	...
	void call(Pointer... pointers)
	{
		...
		if (type instanceof BuiltinMethodCall)
		{
			...
			if (p.isGlobal && p.copy) mo = ScriptTest.globalMemory.get(p.address).copy();
			else if (p.isGlobal) mo = ScriptTest.globalMemory.get(p.address);
