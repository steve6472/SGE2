...
public class PerlinNoise
{
	...
	public double[][] generateSimplexNoise(int width, int height, int seed)
	{
		double[][] simplexnoise = new double[width][height];
		double frequency = 5.0f / (float) width;

		for (int x = 0; x < width; x++)
		{
			for (int y = 0; y < height; y++)
			{
				simplexnoise[x][y] = noise(x * (float) frequency, seed, y * (float) frequency);
	...
	public double[][] generateSimplexNoise(int width, int height, int seed, float freq)
	{
		double[][] simplexnoise = new double[width][height];
		double frequency = freq / (double) width;
	...
	public double[][] generateSimplexNoise(int width, int height, int seed, int octaves)
	{
		double[][] simplexnoise = new double[width][height];
		...
		for (int x = 0; x < width; x++)
		{
			for (int y = 0; y < height; y++)
			{
				simplexnoise[x][y] = smoothNoise(x * (float) frequency, seed, y * (float) frequency, octaves);
				simplexnoise[x][y] = (simplexnoise[x][y] + 1) / 2;
			}
		}
	...
	static
	{
		...
		{ 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148,
				...
				16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22,
