...
public class RenderHelper
{
	...
	public static void renderButton(Screen screen, int x, int y, int width, int height, boolean enabled, boolean hovered)
	{
		if (enabled && hovered)
		{
			renderDoubleBorder(screen, x, y, width, height, BUTTON_HOVERED_COLORS[0], BUTTON_HOVERED_COLORS[1], BUTTON_HOVERED_COLORS[2]);
		} else if (enabled && !hovered)
		{
			renderDoubleBorder(screen, x, y, width, height, BUTTON_ENABLED_COLORS[0], BUTTON_ENABLED_COLORS[1], BUTTON_ENABLED_COLORS[2]);
		} else if (!enabled)
		{
			renderDoubleBorder(screen, x, y, width, height, BUTTON_DISABLED_COLORS[0], BUTTON_DISABLED_COLORS[1], BUTTON_DISABLED_COLORS[2]);
		}
	}
}
